...
texture_2D_dx::texture_2D_dx(texture_setup& texture_setup,
...
texture_2D_dx::texture_2D_dx(int width, int height, std::string& name, Microsoft::WRL::ComPtr<ID3D12Device>& device)
{
    ...
    auto const heapProp = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    attempt(device->CreateCommittedResource(&heapProp, D3D12_HEAP_FLAG_NONE, &texDesc,
